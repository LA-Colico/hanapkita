...
<head>
    <title>Hanap-Kita - Employer Between Dates Report</title>
...
<body>
        ...
        <main id="main-container">
            <div class="content main-content">
                    ...
                    <div class="header-content">
                        <div class="header-left">
                            ...
                            <div class="header-text">
                                <h1>Employer Between Dates Report</h1>
                                <p>Comprehensive analysis of employer registrations within specified date range</p>
                            </div>
                        </div>
                        <?php if ($fdate && $tdate): ?>
                ...
                <div class="stats-grid">
                    ...
                    $sql_total = "SELECT COUNT(*) as count FROM tblemployers WHERE date(RegDtae) between '$fdate' and '$tdate'";
                    $query_total = $dbh->prepare($sql_total);
                    $query_total->execute();
                    $total_employers = $query_total->fetch(PDO::FETCH_OBJ)->count;

                    $sql_active = "SELECT COUNT(*) as count FROM tblemployers WHERE date(RegDtae) between '$fdate' and '$tdate' AND Is_Active = 1";
                    $query_active = $dbh->prepare($sql_active);
                    $query_active->execute();
                    $active_employers = $query_active->fetch(PDO::FETCH_OBJ)->count;

                    $sql_inactive = "SELECT COUNT(*) as count FROM tblemployers WHERE date(RegDtae) between '$fdate' and '$tdate' AND (Is_Active = 0 OR Is_Active IS NULL)";
                    $query_inactive = $dbh->prepare($sql_inactive);
                    $query_inactive->execute();
                    $inactive_employers = $query_inactive->fetch(PDO::FETCH_OBJ)->count;

                    $sql_recent = "SELECT COUNT(*) as count FROM tblemployers WHERE date(RegDtae) between '$fdate' and '$tdate' AND date(RegDtae) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
                ...
                <div class="filters-card">
                    ...
                    <form method="GET" class="filters-grid">
                        ...
                        <div class="filter-group">
                            ...
                            <input type="text" name="search" class="filter-input" placeholder="Search by company name, email..." value="<?php echo htmlentities($search_term); ?>">
                        </div>
                        <div class="filter-group">
                ...
                <div class="employers-card">
                    <div class="employers-header">
                        <h2 class="employers-title">
                            ...
                            <?php if ($total_employers > 0): ?>
                                <span style="color: var(--primary-orange); font-size: 1rem;">(<?php echo $total_employers; ?> found)</span>
                            <?php endif; ?>
                        </h2>
                    ...
                    // Get employers data
                    $sql = "SELECT * FROM tblemployers WHERE date(RegDtae) between '$fdate' and '$tdate' $additional_clause ORDER BY RegDtae DESC LIMIT $limit OFFSET $offset";
                    ...
                    ?>
                        <div class="empty-state">
                            ...
                            <p>No employers registered between <?php echo date('M j, Y', strtotime($fdate)); ?> and <?php echo date('M j, Y', strtotime($tdate)); ?> match your current filters.</p>
                        </div>
                    <?php
                    } else {
                    ?>
                        ...
                        <div class="employer-card">
                            ...
                            <div class="employer-details">
                                ...
                                <div class="employer-detail-row">
                                    ...
                                    <span><?php echo htmlentities($row->CompanyTagline); ?></span>
                                </div>
                                <?php endif; ?>
                                <?php if ($row->industry): ?>
                                <div class="employer-detail-row">
                                ...
                                <div class="employer-detail-row">
                                    ...
                                    <span>Registered: <?php echo date('M j, Y g:i A', strtotime($row->RegDtae)); ?></span>
                                </div>
                            </div>
                            
                            <div class="employer-actions">
                                <a href="view-employer-details.php?viewid=<?php echo htmlentities($row->id); ?>" class="action-btn-small btn-primary" target="_blank">
                        ...
                        <table class="employers-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="date-cell"><?php echo date('M j, Y g:i A', strtotime($row->RegDtae)); ?></td>
                                    <td>
                                        <div style="display: flex; gap: 0.5rem;">
                                            <a href="view-employer-details.php?viewid=<?php echo htmlentities($row->id); ?>" class="filter-btn btn-primary" target="_blank" style="padding: 0.5rem 0.75rem;">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="listed-jobs.php?compid=<?php echo htmlentities($row->id); ?>&&cname=<?php echo htmlentities($row->CompnayName); ?>" class="filter-btn btn-secondary" target="_blank" style="padding: 0.5rem 0.75rem;">
                                                <i class="fas fa-briefcase"></i>
                                            </a>
                                        </div>
                                    </td>
                ...
                <!-- Pagination -->
                <?php if ($total_employers > $limit): ?>
                <div class="pagination-card">
                    <div class="pagination-info">
                        <?php
                        $total_pages = ceil($total_employers / $limit);
                        $start_record = $offset + 1;
                        $end_record = min($offset + $limit, $total_employers);
                        ?>
                        Showing <?php echo $start_record; ?>-<?php echo $end_record; ?> of <?php echo $total_employers; ?> employers
                    </div>
                ...
                <div class="empty-state">
                    ...
                    <p>Please select a date range from the <a href="employer-report.php" style="color: var(--primary-orange);">Employer Report</a> page to view the data.</p>
                </div>
                <?php endif; ?>
            </div>
        </main>
    ...
    <script>
        ...
        function exportToExcel() {
            ...
            if (table) {
                ...
                if (link.download !== undefined) {
                    ...
                    link.setAttribute('download', 'employers_report.csv');
        ...
        const printStyles = `
            @media print {
                .page-header, .filters-card, .export-section, .view-toggle, .employer-actions, .pagination-card { display: none !important; }
